def mean(town, strng)
    (array_with_degree_data(town, strng).inject(:+))/array_with_degree_data(town, strng).length
end
def variance(town, strng)
	sum = 0
	mean_value = mean(town, strng)
	array_data = array_with_degree_data(town, strng)
	array_data.each do |x|
		sum += ((x - mean_value) ** 2)
	end
  return sum/(array_data.length-1)
end
def array_with_degree_data(town, strng)
	town_data = []
	result = []
    town_data = strng.split(/\n/)
    town_data.each do |town_arr|
        if town_arr.downcase.include? town.downcase
          	town_item = town_arr.split(/[\s,']/)
          	town_item.each_with_index do |item, index|
          		if index.odd?
          			result.push(item.to_f)
          		end
          	end
        end
    end
